...
export function httpsPath(host, port, path, headers, { makeRequest }) {
  ...
  function sendRequest(method, body = false) {
    ...
    return new Promise((resolve, reject) => {
      function sendRequestCallback(message, value, etc) {
        // console.log('sendRequest cb', message, value, etc);
        if (message < 0) {
          reject(new Error(`message: ${message} value: ${value} etc: ${etc}`));
      ...
      try {
        ...
        makeRequest(
          {
            ...
            method,
            path,
            // An array containing the HTTP headers to add. Even number elements are header names and odd number elements are the corresponding header values.
            // https://github.com/Moddable-OpenSource/moddable/blob/public/documentation/network/network.md#constructordictionary-2
        ...
        console.log('httpsPath.sendRequest err', method, err, err.message);
        reject(err);
      }
    });
  }
